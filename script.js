...
function renderHero(p) {
  ...
  let resumeBtn = "";
  if (p.resumeUrl) {
    resumeBtn = `<a href="${esc(p.resumeUrl)}" target="_blank" rel="noopener" class="btn btn-outline">${ICONS.download} Resume</a>`;
  }
...
function renderProjectsCta(github) {
...
async function fetchGitHubData(username) {
  ...
  try {
    ...
    const top = repos.filter(r => !r.fork).sort((a, b) => b.stargazers_count - a.stargazers_count).slice(0, MAX_PROJECTS);
...
// Init: load data.json and render everything
async function init() {
  try {
    ...
    renderProjectsCta(data.profile.github);
    fetchGitHubData(data.profile.github);
  } catch (err) {
